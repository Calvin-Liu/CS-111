...
#define MAXDOWNLOADSIZE 1073741824 //Max file size, if it exceeds returns error
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	//Fix me: Part 2
	//Record the size of the file so you can tell how much you want to copy
	//If the copy copied amount is more than FILENAMESIZ then it
	//will overflow the buffer! 
	int actualFileSize;
	actualFileSize = strlen(filename);
	if(actualFileSize > FILENAMESIZ)
		actualFileSize = FILENAMESIZ;
// 	strcpy(t->filename, filename);
 	strncpy(t->filename, filename, actualFileSize);
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	//will overflow the buffer!
	int actualFileSize;
     	actualFileSize = strlen(t->filename);
        if(actualFileSize > FILENAMESIZ)
                  actualFileSize = FILENAMESIZ;
...
//			strcpy(t->disk_filename, t->filename);
			strncpy(t->disk_filename, t->filename, actualFileSize);
		else
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		//Fix me: If the read file name is too large then kill the downlaod
		if(t->total_written > MAXDOWNLOADSIZE)
			error("The file you are trying to download is too large\n"); 
	}
...
static void task_upload(task_t *t)
{
	...
	//Fix me: Part 2
	//second parameter of osp2p_snscanf is the length of file not null terminated
	//Non null terminated so you need to use >=
	//Check to see if length is over 256
	if(t->tail > FILENAMESIZ || strlen(t->buf) > FILENAMESIZ || strlen(t->filename) > FILENAMESIZ)
		error("File size is over 256 bytes, way too big\n");



	//Fix me: Part 2B
	//Need to check to see if the person is uploading/downloading from right directory
	if (t->filename[0] == '/' || ((t->filename[0] == '~') && (t->filename[1] == '/' || t->filename[1] == '\0'))) 
	{
		error("Tried to access other directories\n");
		goto exit;
	}
	int i = 0;
...
/*
static
md5_byte_t  convertCharMD5(char * tempPW){
	//for every character in tempPW, convert from char to unsigned char
	//return a pointer with md5_byte_t
	//create the answer
	md5_byte_t answer[strlen(tempPW)];
		unsigned int i;
		for (i = 0; i < strlen(tempPW); i++)
		{
			answer[i] = (unsigned char) tempPW[i];
		}
	return answer;
}
*/
...
int main(int argc, char *argv[])
{
	...
	if (argc >=3 && strcmp(argv[1], "-l") == 0
		&& osp2p_sscanf(argv[2], "%d", &demo_mode) >=0){
		argc-=2, argv-=2;
		goto argprocess;
	} else if (argc >=2 && argv[1][0] == '-' && argv[1][1] == 'l'
		&& osp2p_sscanf(argv[1], "-l%d", &demo_mode) >=0){
		--argc, ++argv;
		goto argprocess;
...
"         -demo        Design project demo\n");
	...
	//
	// ask the for the password
	char tempPW[100];
	int gimmick = 1;
	printf("please enter password: ");
	scanf("%s", tempPW);
	if (strcmp(tempPW, "abc123") == 0) gimmick = 0;
	//printf("%s\n", tempPW);
	unsigned int i;
	unsigned int size;
	size = strlen(tempPW);
	//initialize the MD5 data
//	md5_byte_t digest[16];
	char * text_digest;
	char tempchar;
	tempchar = 'c';
	text_digest = &tempchar;
	md5_state_t context;
	md5_byte_t temp[size];

//	convert the input string into one usable by the hash
	for(i = 0; i < size; i++){
		temp[i] = (unsigned char)tempPW[i];
	}
	md5_init(&context);
	md5_append(&context, temp , size);
	md5_finish_text(&context, text_digest, 1);
	printf("%s\n", text_digest);
	
//	char * corrAns;
//	corrAns = "PRjgeJYY4uT8GvOn4jPld\0";
//	text_digest[strlen(text_digest)] = '\0';

	decode = 1;

//	if ( strcmp(text_digest, corrAns) != 0)
//	if (strcmp ("R", "R")!= 0)
 	if (gimmick == 1)
	{
		printf("Not the correct password!\nNeeds to match:PRjgeJYY4uT8GvOn4jPld\n");
//		printf("%s\n", tempPW);
		decode = 0;		
	}
	else if (gimmick == 0)
	...
	for (; argc > 1; argc--, argv++)
	{
		if ((t = start_download(tracker_task, argv[1])))
		{
			...
			//let parents fall through apparently, but you still need to
			//keep track of errors in forking
			else if(pid < 0)
			{
				error("Cannot fork a download process\n");
				exit(0);
			}
			else
				task_free(t);
	...
	while ((t = task_listen(listen_task)))
	{
		...
		{
			error("Cannot fork an upload process\n");
			exit(0);
		}
		else
			task_free(t);
